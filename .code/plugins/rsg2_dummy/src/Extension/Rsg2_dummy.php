...
namespace Rsgallery2\Plugin\Content\Rsg2_dummy\Extension;
...
class Rsg2_dummy extends CMSPlugin implements SubscriberInterface
{
...
//	}
	...
	{
		return [
			'onContentPrepare' => 'getRsg2_dummyDisplay'
		];
	}

	public function getRsg2_dummyDisplay (Event $event)
	{
		$context = '';
		$article = '';
		...
		try {
			...
			if (str_contains($article->text, '{rsg2_dummy'))
			{
				$lastUserTestIdx = 0;
...
//				}
				...
				if ($matches)
				{
					...
					foreach ($matches as $usrDefinition)
					{
						...
						// $insertHtml = Rsg2_dummyHelper::galleryImagesHtml();
						$insertHtml = '<h4>--- Rsg2_dummy replacement ---</h4>' . $insertHtml;
			...
			$msg = Text::_('PLG_CONTENT_RSG2_DUMMY') . ' getRsg2_dummyDisplay: '. ' Error (01): ' . $e->getMessage();
	...
	private function extractUserParams(string $usrString)
	{
		...
		try
		{
			...
			$msg = Text::_('PLG_CONTENT_RSG2_DUMMY' . 'extractUserParams: "') . $usrString . '" Error (01): ' . $e->getMessage();
			$app = Factory::getApplication();
			$app->enqueueMessage($msg, 'error');
			return false;
		}
